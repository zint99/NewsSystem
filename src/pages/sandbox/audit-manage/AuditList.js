...
export default function AuditList() {
  ...
  useEffect(() => {
    try {
      (async () => {
        ...
        const url =
          userInfo.roleId === 1
            ? `http://localhost:5000/news?auditState_ne=0&publishState_lte=1&_expand=category`
            : userInfo.roleId === 2
            ? `http://localhost:5000/news?auditState!=0&publishState_lte=1&region=${userInfo.region}&_expand=category`
  ...
  }, [userInfo.roleId, userInfo.region, userInfo.username]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (auditState) => (
        <Tag
          color={
            auditState === 1 ? "orange" : auditState === 2 ? "green" : "red"
          }
        >
          {auditState === 1 ? "审核中" : auditState === 2 ? "已通过" : "未通过"}
        </Tag>
      ),
    },
    {
